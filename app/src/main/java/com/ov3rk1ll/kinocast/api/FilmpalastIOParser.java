...
public class FilmpalastIOParser extends Parser {
    ...
    private ViewModel parseDetail(Document doc, ViewModel model){
        ...
        try {
            ...
            Elements hosts = doc.select("ul.currentStreamLinks a.iconPlay");
            int i = 0;
            for(Element host: hosts){
                String href = "";
                if(!Utils.isStringEmpty(host.attr("data-player-url"))) href =host.attr("data-player-url");
                else if(!Utils.isStringEmpty(host.attr("href"))) href =host.attr("href");
                if(Utils.isStringEmpty(href)) continue;
