...
public class OrderParserActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (Class<?> h: Parser.PARSER_LIST){
            try {
                Parser host = (Parser) h.getConstructor().newInstance();
                mItemArray.add(new Item(host.getParserId(), host.getParserName(), R.drawable.ic_drag_handle_black_48dp));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        mItemArray.add(new Item(NothingParser.PARSER_ID, " - - - - DEACTIVATED - - - - ", R.drawable.ic_drag_handle_black_48dp));

        sortedList = Utils.getWeightedParser(getApplicationContext());
        if(sortedList != null) {
    ...
    protected void onPause() {
        ...
        for(int i = 0; i < mItemArray.size(); i++){
            editor.putInt("order_parser_" + i, mItemArray.get(i).getId());
        }
        editor.putInt("order_parser_count", mItemArray.size());
        editor.commit();
        super.onPause();
    }
